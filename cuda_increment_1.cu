 ...
 */
__global__ void k_axpy_increment_1(int n, float a, float *x, float *y) {
  int n_threads = blockDim.x * gridDim.x;
  int thread_id = blockDim.x * blockIdx.x + threadIdx.x;
  int index_begin     = ((int64_t)n* thread_id     )/n_threads;
  int index_end       = ((int64_t)n*(thread_id + 1))/n_threads;
  int i;
...
int main(int argc, char **argv) {
  ...
  /* warm-up run */
  k_axpy_increment_1<<< nb, nt >>>(length, 1.0, d_x, d_y);
  /* begin timing */
